"""Reproductions of upstream pyarrow bugs affecting ``null``-typed nested fields.

nested-pandas represents an all-null nested sub-column with pyarrow's ``null``
data type. Two open pyarrow bugs corrupt such fields, and nested-pandas has to
work around them:
...
* https://github.com/apache/arrow/issues/43838 -- casting a ``null``-typed list
  ...
  truncated to the row count instead of the flat element count).
...
These tests assert the *correct* behavior, so they fail while the upstream bugs
are present. They act as canaries: once pyarrow fixes the bugs, they start
passing and the corresponding nested-pandas workarounds can be removed.
"""
